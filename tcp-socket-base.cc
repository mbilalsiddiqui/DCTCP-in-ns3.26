...
TcpSocketBase::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketBase")
    ...
    .AddAttribute ("MinRto",
                   ...
                   TimeValue (MilliSeconds (10)), // RFC 6298 says min RTO=1 sec, but Linux uses 200ms.
...
//
    ...
    .AddAttribute ("useDCTCP", "DCTCP socket",
                    BooleanValue (false),
                    MakeBooleanAccessor (&TcpSocketBase::m_DCTCP),
                    MakeBooleanChecker ())

     .AddAttribute ("DctcpGainFactor",
                     "Weight 'g' given in DCTCP",
                     DoubleValue (1.0 / 16.0),
                     MakeDoubleAccessor (&TcpSocketBase::m_g),
                     MakeDoubleChecker<double> (0.0, 1.0))

     .AddTraceSource ("DctcpAlphaTracker",
                       "DCTCP Alpha Trace Source",
                       MakeTraceSourceAccessor (&TcpSocketBase::m_alpha),
...
/****************************************************************************************************************/				   
//					 
// Not Used Currently. But if someone wants to add trace. They can use it to trace these parameters.					 
...
/****************************************************************************************************************/				   
    ...
    //--------------------------DCTCP Related Paremeters Set Here for Default Values-----------------//
    m_g (1.0 / 16.0),
    m_alpha (1.0),
    m_BytesAckedWithEcn (0),
    m_TotalAckedBytes (0),
    m_alphaTrack (0),
    m_MaxSeq (0),
...
/****************************************************************************************************************/				   
    ...
    //------------------------DCTCP Related Settings--------------------------------------------------//
    m_g (sock.m_g),
    m_alpha (sock.m_alpha),
    m_BytesAckedWithEcn (sock.m_BytesAckedWithEcn),
    m_TotalAckedBytes (sock.m_TotalAckedBytes),
    m_alphaTrack (sock.m_alphaTrack),
    m_MaxSeq (sock.m_MaxSeq),
    m_DCTCP (sock.m_DCTCP),         
    m_ecnTransition (sock.m_ecnTransition),
    m_ecnState (sock.m_ecnState),
    m_ecnEchoSeq (sock.m_ecnEchoSeq),
    m_congestion (sock.m_congestion)
...
TcpSocketBase::DoConnect (void)
{
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK || m_state == CLOSE_WAIT)
    { // send a SYN packet and change state into SYN_SENT
      ...
      m_state = SYN_SENT;
      m_ecnState &= ~ECN_CONN;    // because sender is not yet aware about receiver's ECN capability
    }
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if (header.GetEcn() == Ipv4Header::ECN_CE)
    {
      // IP packet with marked CE bit was received at the receiver
      m_congestion = true;
    }
    ...
    case CLOSED:
      ...
      if ((tcpHeader.GetFlags () &
        ...
        { // Since m_endPoint is not configured yet, we cannot use SendRST here
          ...
          // Sender Tells here to set ECT bits at switch (ECN)
          if (m_ecnState & ECN_CONN)
            {
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (ackNumber == m_txBuffer->HeadSequence ()
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer
      ...
      if ((m_ecnState & ECN_CONN) && (tcpHeader.GetFlags () & TcpHeader::ECE))
        {
          // ECN Echo is marked in TCP header
          if (m_ecnEchoSeq < tcpHeader.GetAckNumber ())
            {
              // ECN echo number received is valid
                              ...
                              const Address& fromAddress, const Address& toAddress)
{
  ...
  // Part of ECN Handshaking Process:
...
TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (tcpflags == 0)
    { // Bare data, accept it and move to ESTABLISHED state. This is not a normal behaviour. Remove this?
      ...
      NS_LOG_INFO ("Opposite doesn't support ECN.");
      m_ecnState &= ~ECN_CONN;
                               ...
                               const Address& fromAddress, const Address& toAddress)
{
  ...
  if (tcpflags == 0
    ...
    { // Probably the peer lost my SYN+ACK
      ...
      // Check if we received an ECN SYN packet.
      if (m_ecn && (tcpHeader.GetFlags () & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE))
        {
          NS_LOG_INFO ("Received ECN SYN packet");
          m_ecnState |= ECN_CONN;
                             ...
                             const Address& fromAddress, const Address& toAddress)
{
  ...
  // Check if we received an ECN SYN packet. Change the ECN state of receiver to ECN_IDLE if sender has sent an ECN SYN  packet and the traffic is ECN Capable. Handshaking Process
...
//
  if (m_ecnState & ECN_SEND_CWR)
    {
      // Send CWR marked packet. This Establishes that Sender will now reduce Congestion Window either half(ECN) or according to DCTCP
      flags |= TcpHeader::CWR;
  ...
  if (GetIpTos () || (m_ecnState & ECN_CONN))
    {
      ...
      //Setting ECT bits in retransmitted packet. ns-2 like? Might look further into this
      if (m_ecnState & ECN_CONN)
        {
          tos |= Ipv4Header::ECN_ECT0;
        }
...
//
  ...
  if(m_DCTCP)
  {
    // highTxMark is Highest Sequence Number Sent.
    m_MaxSeq =std::max (std::max (seq + sz, m_tcb->m_highTxMark.Get ()), m_MaxSeq);
  }
//
//
//
...
//
     if ((m_ecnState & (ECN_RX_ECHO | ECN_SEND_CWR)) == ECN_RX_ECHO)
        {
          // Set CWR and reduce window if according to either DCTCP is used or just half if no DCTCP is used.
...
//
//
// Method to calculate alpha for DCTCP in one RTT

void
TcpSocketBase::AlphaChangeRtt (const TcpHeader &tcpHeader)
{
  // Current Ack Number - Highest Ack Received
  ...
  if (ackedBytes > 0)
    {
      ...
      if (tcpHeader.GetFlags () & TcpHeader::ECE)
        {
          m_BytesAckedWithEcn += ackedBytes;
        }
    }
  // Updating Alpha once for every RTT:
  if (tcpHeader.GetAckNumber () > m_alphaTrack)
    {
      m_alphaTrack = m_MaxSeq;

      m_alpha = (1 - m_g) * m_alpha + m_g * ((double)m_BytesAckedWithEcn / (m_TotalAckedBytes ? m_TotalAckedBytes : 1));

      //Setting The Acked Bytes ZERO For Resue in Each RTT.  
      m_BytesAckedWithEcn = 0;
      m_TotalAckedBytes = 0;
    }
}
...
//  
  if(m_DCTCP)
  {
    m_alphaTrack = m_MaxSeq = m_tcb->m_nextTxSequence;
...
//
  if(m_DCTCP)
  {
    ...
    if (m_ecnState & ECN_CONN)
    {
      if (m_ecnTransition)    // Mark ECE flag in TCP Packet Once CE bit is set from IP header (In method Update ECN state below)
        {
          if (!(m_ecnState & ECN_TX_ECHO))
            {
              NS_LOG_DEBUG ("Sending ECN Echo.");
              dcflag |= TcpHeader::ECE;
            }
          m_ecnTransition = false;
        }
...
/************************************************************************************************************************************************/				   
  ...
  if ((m_ecnState & ECN_CONN) && (m_ecnState & ECN_TX_ECHO))
    {
...
void
TcpSocketBase::UpdateEcnState (const TcpHeader &tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
  if(!m_DCTCP){
  if ((tcpHeader.GetFlags () & TcpHeader::CWR) && (m_ecnState & ECN_TX_ECHO))
    {
      NS_LOG_INFO ("Transmitter Halved the CWND. Stop sending ECN Echo.");
      m_ecnState &= ~ECN_TX_ECHO;
    }
  if (m_congestion && !(m_ecnState & ECN_TX_ECHO))
    {
...
//
  ...
  if(m_DCTCP)
  {
   ...
   // Check if Congestion was experienced (bit set from IP header) and See if its a receiver then start sending ECN Echo and Update the state from Idle to ECN Transmitting ECHO from Receiver Side
  if (m_congestion && !(m_ecnState & ECN_TX_ECHO))
    {
      NS_LOG_INFO ("Congestion was experienced. Start sending ECN Echo.");
      m_ecnState |= ECN_TX_ECHO;
   ...
   //Stop Sending Once Sender Has Adjusted the Sending Rate: 
  else if (!m_congestion && (m_ecnState & ECN_TX_ECHO))
    {
      m_ecnState &= ~ECN_TX_ECHO;
      m_ecnTransition = true;
      m_delAckCount = m_delAckMaxCount;
...
TcpSocketBase::DecreaseWindow (void)
{
  ...
  if(!m_DCTCP){
  NS_LOG_FUNCTION (this);
  //cout << m_tcb->m_cWnd.Get()<<endl;
  cout << Simulator::Now ().GetSeconds ()<< " ECN Received: Congestion Window at Sender Before ECN reaction:  "<< m_tcb->m_cWnd.Get() << endl;
  m_tcb->m_ssThresh = m_congestionControl->GetSsThresh (m_tcb, BytesInFlight ());
  m_tcb->m_cWnd = std::max (m_tcb->m_cWnd.Get () / 2, GetSegSize ());
  //cout << m_tcb->m_cWnd.Get() << endl;
  cout << Simulator::Now ().GetSeconds ()<< " Congestion Window halved: "<< m_tcb->m_cWnd.Get() << endl;
  cout << "\n" << "**********************************************************************************" << endl;
  }
...
//  
  if(m_DCTCP)
  {
   // Decreasing Window Once Every RTT  
  cout << Simulator::Now ().GetSeconds ()<< " Congestion Window Before:   "<< Window() << endl;

  uint32_t newCwnd = (1 - m_alpha / 2.0) * Window ();


  cout << Simulator::Now ().GetSeconds ()<< " Congestion Window Reduced According to Alpha "<< m_alpha << " is "  << newCwnd << endl;
  cout << "\n" << "***************************************************************************************************" << endl;
...
TcpSocketBase::MarkEmptyPacket (void) const
{
  ...
  if(!m_DCTCP) return m_ecnState & ECN_CONN;
/************************************************************************************************************************************************/				   
//
//
  if(m_DCTCP) return m_ecn;
//
//
/************************************************************************************************************************************************/				   
